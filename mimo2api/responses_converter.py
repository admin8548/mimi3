...
class RespCompactionItem(BaseModel):
    # Codex accepts `compaction` and aliases `compaction_summary`; keep
    # `context_compaction` parseable for compacted-history replay.
...
def _parse_response_input_item(raw_item: Any) -> RespItem | None:
    ...
    if item_type in {"compaction", "compaction_summary", "context_compaction"}:
        ...
        if item.get("encrypted_content") is None:
            # Keep parsing tolerant; an empty context_compaction marker is valid
            # in some Codex histories and should simply be ignored downstream.
            item["encrypted_content"] = ""
        return RespCompactionItem.model_validate(item)

    if item_type == "compaction_trigger":
        # This is a control item for Codex remote compaction v2 and is handled
        # by the gateway route before normal Responses conversion.
        return None
...
def convert_request(req: dict[str, Any]) -> dict[str, Any]:
    ...
    # 只保留 Chat Completions 标准字段，移除所有 Responses API 专有字段
    # （reasoning, text, include, metadata 等会导致上游 API 返回 400 错误）
    ALLOWED_CHAT_KEYS = {
        "model", "messages", "stream", "max_tokens",
        "temperature", "top_p", "frequency_penalty", "presence_penalty",
        "n", "stop", "tools", "tool_choice", "response_format",
        "seed", "logprobs", "top_logprobs", "user",
    }
    ...
    # Strip internal conversion fields from messages before forwarding
